...
$files_path_common_absolute = str_replace("\\", "/", $files_path_common_absolute);
$files_path_common_absolute = $files_path_common_absolute . 'common';
$dir = $files_path;
...
include $_SERVER['DOCUMENT_ROOT'].'\local\oslist.php';
...
if (($CurrOS=="Linux")&&($android=="")&&($mac=="")) echo "<img src=/doc/files/common/linux.png' title='You are running Linux'>&nbsp;";
...
&nbsp;<a href="zipfolder.php?fname=<?php echo $prjname; ?>&targetdir=<?php echo realpath($dir); ?>&debug=12345678"><img src="/doc/files/common/images/compress-icon.png" title="Download this folder zipped"/></a>&nbsp;
<!--
			...
			if(!is_dir($dir.'/'.$file)) {		// don't display directories
				...
				if( /*($rest==".BAT") || */($rest==".RUN") || ($rest==".LNK")) {							// display only .bat files
					...
					if (file_exists ( $dir.'/.'.$file )) {
						...
						// windows psexec icon
							.'<a href="psexec.php?targetdir='.realpath($dir).'&targetfile='.$file.'&urldir='.$urldir.'&host='.$host.'"  onclick=\'psexec(this.href); return false;\' ><img src="/doc/files/common/images/Windows16.png" title="System" /></a>'
				...
				if(($rest==".SH") || ($rest==".RN")) {							// display only .bat files
					...
					if (file_exists ( $dir.'/.'.$file )) {
						...
						echo '<a class="linuxlink" href="/doc/files/common/downloadfile.php?fname='.$file.'&targetdir='.realpath($dir).'&term=gnome-terminal"  onmouseover="getFileFromServer('.'\'.'.$file.'\''.', function(text){ show(text, this, -1)});">'.$file.'</a>'
							...
							.'<a class="winlink" href="runinlinux.php?targetdir='.realpath($dir).'&targetfile='.$file.'&urldir='.$urldir.'&host='.$host.'"  onclick=\'psexec(this.href); return false;\' ><img src="/doc/files/common/images/linux.png"/></a>'
